...
ネットマスクは`255.255.255.0`と設定してください．
ゲートウェイは設定する必要はありません．
...
ヘッダーとして`SNSE`が，フッターとして`:`がついてきます．また，`,`で区切った6つのデータが受信できます．最初の３つのデータは力で，順番はｘ，ｙ，ｚであり，単位は`N`です．残りの３つのデータはトルクで，順番はｘ，ｙ，ｚ軸周りとなります．単位は`Nm`です．
...
## 6. その他
### 6.1 その他のプログラム
`ros_massage_bed/src`にはマッサージベッドを動かす際に必要なプログラム以外にも，いくつかプログラムが保存されています．何かの参考になるかもしれないので，念の為，説明しておきます．

- `simple_force_massage.cpp`

  このプログラムは，マッサージのデモで使用しました．人間をsin波でマッサージします．力覚センサを使用して手先反力を計測し，P制御により，与える力が一定になるように制御します．ただ，Pゲインを適当に設定したため，振動します．マッサージ的には丁度いいかもしれませんが．．また，手先位置，力，速度指令をログとして記録します． 実行方法は下記のとおりです．
  ```
  roslaunch tm5_900_moveit_config tm5_900_moveit_planning_execution.launch sim:=false robot_ip:=192.168.10.2
  ```
  ```
